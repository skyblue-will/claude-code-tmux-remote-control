...
#                                       session exactly, so the app/URL maps to
#                                       a name you chose (vs a generated one).
#      --dangerously-skip-permissions   don't prompt for each tool call — the
#                                       session runs unattended. This is what
#                                       makes headless work. Use ONLY in a
#                                       trusted / sandboxed environment.
CMD="claude --remote-control \"$NAME\" --dangerously-skip-permissions"

# 3) Feed the first prompt — THE BIT PEOPLE GET WRONG.
...
#    no command substitution firing. Without it, a space splits the prompt and a
#    $(...) would EXECUTE before Claude ever sees it.
if [[ -n "$SEED" ]]; then
